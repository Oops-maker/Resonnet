        ...
        if topic_dir.is_dir():
            ...
            if topic:
                if topic.discussion_status == DiscussionStatus.RUNNING:
                    topic.discussion_status = DiscussionStatus.FAILED
...
def create_topic(data: TopicCreate) -> Topic:
    ...
    topic = Topic(
        ...
        session_id=topic_id,
        title=data.title,
        body=data.body if data.body else "",  # allow empty body
...
def update_topic_discussion(
    ...
    if not topic:
        return None
    topic.discussion_status = status
    if result:
        topic.discussion_result = result
    topic.updated_at = utc_now_iso()
    _save_topic_to_file(topic)
    return topic
