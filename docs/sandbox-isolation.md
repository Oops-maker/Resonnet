...
**Date**: 2026-02-19 (updated 2026-07-25)
**Status**: Implemented (Phase 3 — sandbox-runtime integration)
...
## 8. Phase 3: sandbox-runtime (srt) Integration

### 8.1 What Changed

Phase 3 replaces the custom Seatbelt/bwrap code with Anthropic's official
[sandbox-runtime](https://github.com/anthropic-experimental/sandbox-runtime)
(`srt`), a lightweight OS-level sandboxing tool.

**Backend priority** (highest first):
1. **srt** — Anthropic-maintained, JSON-based config, supports filesystem
   *and* network isolation
2. **sandbox-exec** — macOS legacy fallback (custom Seatbelt profile)
3. **bwrap** — Linux legacy fallback (custom bubblewrap command)

Set `SANDBOX_USE_SRT=false` to skip srt and fall back to the legacy backend.

### 8.2 Benefits Over Phase 2

| Aspect | Phase 2 (custom) | Phase 3 (srt) |
|--------|-------------------|---------------|
| Config format | SBPL template / bwrap flags | Declarative JSON |
| Network isolation | Not supported | Domain allowlist (disabled by default) |
| MCP server sandbox | Not supported | Each server wrapped with srt |
| Maintenance | Self-maintained | Anthropic-maintained upstream |
| Mandatory denies | Manual | Built-in (.bashrc, .gitconfig, etc.) |

### 8.3 Configuration

Per-invocation config is generated by `app/agent/srt_config.py`:

- `build_srt_settings()` — Main agent sandbox (workspace + IPC writable)
- `build_mcp_srt_settings()` — MCP server sandbox (only `shared/` writable)
- `write_srt_settings_file()` — Write config to temp JSON file

### 8.4 MCP Server Sandboxing (New)

Each MCP server command is automatically wrapped with srt:

```
Before: {"command": "npx", "args": ["@example/mcp-server"]}
After:  {"command": "srt", "args": ["--settings", "<config>", "npx", "@example/mcp-server"]}
```

MCP servers get a tighter policy than the main agent:
- Write limited to `workspace/shared/` only (not full workspace)
- Same sensitive-path deny list

### 8.5 Network Isolation (Future)

Network isolation is prepared but not enabled. To enable it in the future:

1. Set `allowed_domains` parameter in `build_srt_settings()` calls
2. Configure the domain allowlist (e.g., `["api.anthropic.com"]`)
3. The `enableWeakerNetworkIsolation` flag will automatically switch to `false`
...
```
app/agent/
├── srt_config.py          ← NEW: srt settings generator
├── sandbox_exec.py        ← MODIFIED: srt as primary backend
├── sandbox_runner.py      ← UNCHANGED: runs inside sandbox
├── discussion.py          ← MODIFIED: MCP servers wrapped with srt
├── topic_sandbox.py       ← UNCHANGED: audit/concurrency layer
└── expert_reply.py        ← UNCHANGED: uses sandbox_exec

tests/
└── test_srt_config.py     ← NEW: 19 unit tests for srt config
```

### 8.8 Docker Dependencies

The Dockerfile now installs:
- `bubblewrap` — srt's Linux sandbox backend
- `socat` — srt's network proxy dependency
- `ripgrep` — srt dependency
- `Node.js 20.x` + `@anthropic-ai/sandbox-runtime` — srt CLI
